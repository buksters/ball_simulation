...
namespace PathCreationEditor
{
    public static class MouseUtility
    {
        ...
        public static Vector3 GetMouseWorldPosition(PathSpace space, float depthFor3DSpace = 10)
        {
            ...
            // Mouse can only move on YZ plane 
            else if (space == PathSpace.yz)
            {
                var xDir = mouseRay.direction.x;
                if (xDir != 0)
                {
                    var dstToYZPlane = Mathf.Abs(mouseRay.origin.x / xDir);
                    worldMouse = mouseRay.GetPoint(dstToYZPlane);
                }
            }
