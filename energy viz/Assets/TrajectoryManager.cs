...
public class TrajectoryManager : MonoBehaviour
{
    ...
    private PhysicsScene _physicsSim;
    [SerializeField]
    ...
    private void Start()
    {
        ...
        CreateSceneParameters _param = new CreateSceneParameters(LocalPhysicsMode.Physics3D); //define the parameters of a new scene, this lets us have our own separate physics 
        _simScene =  SceneManager.CreateScene("Simulation",_param); // create a new scene and implement the parameters we just created
        _physicsSim = _simScene.GetPhysicsScene(); // assign the physics of the scene so we can simulate on our own time. 
    ...
    public void SimulateLaunch(Transform player, Vector3 force)   //call this every frame while player is grabed;
    {
        ...
        _simulatedObject.GetComponent<Rigidbody>().velocity = Vector3.zero; // resets sim objects velocity to 0;



        if (_lastForce != force) //if force hasnt changed, skip simulation;
        { 
            _simulatedObject.GetComponent<Rigidbody>().AddForce(force); //simulate the objects path
